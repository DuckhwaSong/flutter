...
import 'package:device_info_plus/device_info_plus.dart';          // 추가 필요
...
class _MyAppPageState extends State<MyAppPage> {
  ...
  if (Platform.isAndroid) {
    ...
    // 안드로이드 13 (SDK 33) 이상인 경우
    if (androidInfo.version.sdkInt >= 33) {
      // 최신 폰은 storage가 아니라 photos, videos 권한을 체크해야 합니다.
      var videoStatus = await Permission.videos.status;
      
      if (!videoStatus.isGranted) {
        // 비디오 권한을 요청 (유튜브 다운로더는 보통 비디오 권한이면 충분합니다)
  ...
  return true; // iOS나 다른 플랫폼은 일단 true 반환
}
  ...
  Future<String> _getPublicDownloadPath() async {
    String? downloadDirPath = null;
      ...
      if (Platform.isAndroid) {
        downloadDirPath = await ExternalPath.getExternalStoragePublicDirectory(ExternalPath.DIRECTORY_DOWNLOAD);
  ...
  Future<bool> _downloadMedia(var stream) async {
    ...
    EasyLoading.showProgress(0.3, status: "Now download streamFile : ${streamFile}...");
    ...
    EasyLoading.showProgress(0.3, status: "Now download path : ${tempDir_path}...");
    ...
    EasyLoading.showProgress(0.3, status: "Now download streamTitle : ${streamTitle}...");
